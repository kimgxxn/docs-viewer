...
- **접기 블록** — `<details><summary>제목</summary>` … `</details>` 로 감싼 부분은 접힌 채로
  나오고, 제목을 누르면 펼쳐집니다. 안쪽 내용은 마크다운으로 그대로 렌더되므로
  mermaid 다이어그램·표·목록을 넣어도 됩니다 (세로로 긴 그림을 감출 때 씁니다)
- **다이어그램·이미지 확대 보기** — 다이어그램 오른쪽 위 `⤢ 크게` 버튼(또는 다이어그램
  더블클릭, 이미지는 클릭)으로 전체 화면 뷰어를 엽니다. 마우스 휠(트랙패드 핀치)로
  확대·축소, 드래그로 이동, 더블클릭으로 맞춤 ↔ 원본 크기 전환.
  단축키는 `+` `-` 확대·축소 · `0` 화면 맞춤 · `1` 원본 크기 · 방향키 이동 · `esc` 닫기.
  SVG 라서 아무리 키워도 글자가 깨지지 않습니다 (큰 메뉴 지도·ERD 를 볼 때 씁니다)
...
- 오른쪽 목차도 왼쪽 경계를 드래그해 **너비 조절** (더블클릭 = 기본값 230px, 기억됨).
  제목이 길어 `…` 로 잘릴 때 늘려서 씁니다
- 단축키: `/` 검색 · `b` 사이드바 · `t` 목차 · `w` 본문 폭 · `r` 새로고침 · `s` 북마크 · `esc` 닫기
  (확대 보기 중에는 `+` `-` `0` `1` · 방향키 · `esc`)
